      ...
      if (w.is_bay) {
        ...
        for (const sec of sections) {
          rows.push({
            ...
            ArchitraveWidth: 0,
            ArchitraveDrop: Number(m.outside_length) || 0,
        ...
        const remoteJobs = remoteRows.filter((row) => !unsyncedIds.has(row.id)).map((row) => {
          ...
          const windows = windowData.map((rw) => {
            ...
            return {
              ...
              has_cornices: rw.has_cornices ?? lw?.has_cornices ?? false,
              wall_to_wall: rw.wall_to_wall ?? lw?.wall_to_wall ?? false,
              opening: rw.opening ?? lw?.opening ?? "",
              operating_method: rw.operating_method ?? lw?.operating_method ?? "",
              control_side: rw.control_side ?? lw?.control_side ?? "",
